...
An ordinary `Sense` is derived only when every structural alternative has examined the block with an applicable verdict that decided one way or the other, and there is no structural conflict. Complete positive partitions were already verified when records were authored and are revalidated when persisted records are materialized.
...
Coverage is computed from the current code-defined population and applicable records. Each pass reports population size and hash, its examined, positive, negative and unresolved counts, and the number of stale records.
...
A resolved node's raw span is a container extent: it marks how far the node reaches, and a parent may therefore span nested structural children. Direct textual content is carried separately as ordered inline segments; punctuation between form/gloss constituents is carried explicitly as a separator rather than silently attached to either constituent.
...
- qualification and grammatical axis, norm, and target
