...
A marker governs one target, and a second scope for a marker already scoped in the same decision fails closed at authoring time.
...
Malformed persisted geometry is an integrity error rather than a stale verdict. The store also rejects duplicate record ids, multiple records for the same locator in one pass, and records that name no decision procedure.
