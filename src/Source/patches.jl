...
function minimal_edit(old::AbstractString, new::AbstractString)::Tuple{Int, Int}
	old_chars = collect(old)
	new_chars = collect(new)
	shortest = min(length(old_chars), length(new_chars))
	prefix = 0
	while prefix < shortest && old_chars[prefix + 1] == new_chars[prefix + 1]
		prefix += 1
	end
	suffix = 0
	while suffix < shortest - prefix &&
			old_chars[end - suffix] == new_chars[end - suffix]
		suffix += 1
	end
	prefix_bytes = sum(ncodeunits, @view(old_chars[1:prefix]); init = 0)
	suffix_bytes = sum(ncodeunits, @view(old_chars[(end - suffix + 1):end]); init = 0)
...
function apply_patches(raw::String, file::AbstractString, patches::Vector{Patch})
	...
	for patch in patches
		...
		replacement = slice(patch.new, prefix + 1, ncodeunits(patch.new) - suffix + 1)
	...
	for (raw_start, raw_end, replacement, line) in staged
		carried = slice(raw, cursor, raw_start)
	...
	write(buffer, slice(raw, cursor, ncodeunits(raw) + 1))
	return (String(take!(buffer)), edits)
end
